...
**--cache-directory DIR**::
	Cache directory tracking images.
...
**DOSH_MOUNT_OPTIONS**::
	Set bind mount volume options.
	Equivalent to --mount-options <OPTIONS>.
...
**Running**:
	Image with *Dockerfile(5)* unchanged since built and a container is
	running.
...
Specify the *Dockerfile(5)* to use
	...
	$ dosh --dockerfile docker/tests/Dockerfile.fedora -c "grep '^NAME=' /etc/os-release"
	sha256:44d9de323a55
	NAME=Fedora
