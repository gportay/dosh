...
echo -e "\e[1mRunning $test...\e[0m"
if          echo 'whoami; echo "$USER" "$0" "$#" "$@"' | dsh "$@" -s | tee /dev/stderr | \
   diff - <(echo 'whoami; echo "$USER" "$0" "$#" "$@"' | $SHELL   -s | tee /dev/stderr )
then
	ok=$((ok+1))
	echo -e "\e[1m$test: \e[32m[OK]\e[0m"
else
...
echo -e "\e[1mRunning $test...\e[0m"
if          echo 'whoami; echo "$USER" "$0" "$#" "$@"' | dsh "$@"  -s "one" "two" "three" | tee /dev/stderr | \
   diff - <(echo 'whoami; echo "$USER" "$0" "$#" "$@"' | $SHELL    -s "one" "two" "three" )
then
	ok=$((ok+1))
	echo -e "\e[1m$test: \e[32m[OK]\e[0m"
else
...
echo -e "\e[1mRunning $test...\e[0m"
if          echo 'whoami; echo "$USER" "$0" "$#" "$@"' | dsh "$@"  -s "one + two" "three" | tee /dev/stderr | \
   diff - <(echo 'whoami; echo "$USER" "$0" "$#" "$@"' | $SHELL    -s "one + two" "three" | tee /dev/stderr )
then
	# See FIXME in dsh
	# ok=$((ok+1))
	fix=$((fix+1))
